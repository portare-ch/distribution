...
images(){
  image_locations | sed -nE 's|^.*PortareOS-([^.]*)\..*-[0-9]+(-.*)?.img.gz|\1\2|p'
}

find_img(){
  DEVICE="${1%-*}"
  SUBDEVICE="${1#${DEVICE}}"
  image_locations | grep -m 1 "/PortareOS-${DEVICE}.aarch64-.*${SUBDEVICE}.img.gz" || exit 1
}
