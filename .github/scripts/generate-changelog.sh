...
while IFS=$'\x1f' read -r subject author commit_hash; do
  ...
  case "$prefix" in
    ...
    # Emulators & Frontend
    *-sa|*-lr|retroarch|emulationstation|gamecontrollerdb|portareos-systems|hypseus*|openbor*|pico-8*|heroic*|amiberry*|mednafen*|steam*|box64|box86|fex-emu|wine|portmaster*|moonlight*|lowerdeck*|themes*|es-themes*|oga*|m8c*|supersnes9x|pcsx2)
    ...
    # System
    busybox|systemd*|deviceinfo|post-update|updateabl|options|lib32|install*|build|portareos*|wayland*|libinput|libxkbcommon|weston*|mtdev|dbus|udevil|parted|util-linux|e2fsprogs|dosfstools|kmod|libusb*|usbutils|fuse*|procps*|nano|evrepeat|emmctool|wait-time-sync|inputplumber*|bash|btop|btrfs-progs|drm_tool|gptfdisk|i2c-tools|lsof|nvtop|powerstate|squashfs*|squashfuse|system-utils|umtprd|usb-modeswitch|openssl|libgpg-error|libarchive|libzip|p7zip|gzip|xz|cabextract|expat|icu|jsoncpp|libxml2|libcroco|libiconv|boost|glib|glibc|ncurses|readline|cmake|joyutils|gamepadcalibration|entware|socat|portareos-splash|coreutils|dialog|freeimage|pyudev|miniupnpc|sound|poppler|grep|file|sleep|avfs|libiio|bin2c|libserialport|swig|llvm|gcc*|go|lua*|nasm|rust*|cargo*|cbindgen|textviewer|dejavu|spleen*|apitrace|strace|at-spi2-atk|at-spi2-core|autostart|config|profiled|sources|sysctld|udevd|bdf2psf|shared-mime-info|ccache|ecm|gnulib|inih|json-glib|libaio|libcom-err|libdatrie|libfmt|libpthread-stubs|libthai|make|patchelf|six|xa|commander|control-gen|device-switch|gamepadtester|jstest-sdl|list-guid|mako-osd|qterminal|sdljoytest|sdltouchtest|compositor|swaywm-env|empty|modules|pyfdt|qemu|synctools|usbgadget|panel|input_sense)
      category="🔧 System" ;;
  esac
  fi
