# The color profile: sRGB, D65, gamma 2.2 on the Nova's panel
...
**Settings > Color profile** switches between *stock* and *sRGB 2.2*. The setting is `display.colorprofile` in `system.cfg`, the profile is `/usr/config/color/srgb22.profile`, and the launcher writes it into the display controller (portarelauncher `color.c`, `kms.c`). It holds for everything: every emulator, mpv, Steam, the launcher itself.
...
`projects/PortareOS/packages/portareos/sources/color/fit-srgb22.py`, from a checkout of their repository:
...
2. **The correction for our pipeline.** A 3×3 matrix and a 22-knot monotone curve per channel, optimised together to minimise CIEDE2000 against sRGB primaries, D65, gamma 2.2 — their targets and their equations (`tools/color_math.py`) — over the 36 boundary colors, the 56 reference patches, a 6×6×6 grid and a gray ramp, from eight starting points.
